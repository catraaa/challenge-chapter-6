      ...
      setFilteredCars(
        getCarsResult.filter((car) => {
          if (passenger === "") {
            ...
            if (driverType === "withDriver") {
              return car.available === true && Date.parse(car.availableAt) < Date.parse(inputDateTime) && car.capacity >= passenger;
            } else {
              return car.available === false && Date.parse(car.availableAt) < Date.parse(inputDateTime) && car.capacity >= passenger;
            }
          }
        })
      );
                ...
                <Col md={11} className="p-0">
                  <Row className="row-cols-sm-2 row-cols-lg-4 row-cols-1">
                    <Col className="pe-3 ps-0">
                      ...
                      <Form.Select name="selectDriverType" value={driverType} onChange={(event) => setDriverType(event.target.value)} required>
                    ...
                    <Col className="pe-3 ps-0">
                      ...
                      <input className="form-control" type="date" name="inputDate" value={rentDate} onChange={(event) => setRentDate(event.target.value)} required />
                    </Col>
                    <Col className="pe-3 ps-0">
                      <Form.Label htmlFor="inputTime">Waktu Jemput/Ambil</Form.Label>
                      <input className="form-control" type="time" name="inputTime" value={rentTime} onChange={(event) => setRentTime(event.target.value)} required />
                    </Col>
                    <Col className="pe-3 ps-0">
                      <Form.Label htmlFor="inputPassenger">Jumlah Penumpang (optional)</Form.Label>
                      <input className="form-control" type="text" name="inputPassenger" placeholder="Jumlah Penumpang" value={passenger} onChange={(event) => setPassenger(event.target.value)} />
                    </Col>
                  </Row>
                </Col>
                <Col sm={1} className="p-0 d-flex align-items-end">
                  <MyButton type="submit" className="button-global">
                    Cari Mobil
                  </MyButton>
                </Col>
              </Row>
